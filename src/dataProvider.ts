...
import { ORDERS_SOURCE_NAME, USERS_SOURCE_NAME } from '../constants/sourceNames'
...
export default {
  ...
  getOne: async (resource, params) => {
    ...
    if (data.sizes && typeof data.sizes === 'object' && Object.values(data.sizes).length) {
      let sizes: string[] = []
  ...
  update: async (resource, params) => {
    if (resource === USERS_SOURCE_NAME) {
      // Существующая логика обновления пользователя
    ...
    if (resource === ORDERS_SOURCE_NAME) {
      const response = await api.post('/admin/edit-order', {
        id: params.data._id,
        email: params.data.email,
        status: params.data.status,
      });
    
      // response.data = { status: 200, updatedOrder: { ... } }
      ...
      return {
        data: {
          ...updatedOrder,
          id: updatedOrder._id,  // Переименование _id в id
        },
      };
    }
  
    // Логика обновления для товаров (как у вас была)
    const sizes = {} as { [index: string]: string }
